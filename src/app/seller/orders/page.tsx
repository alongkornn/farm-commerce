  ...
  return (
    <OperationsShell mode="seller" title="คำสั่งซื้อ" description="อัปเดตสถานะการเตรียมสินค้าและข้อมูลจัดส่ง">
      <DataTable headers={["เลขที่", "ลูกค้า", "วันที่", "ยอดรวม", "สถานะ", "ดำเนินการ"]}>
        {mockOrders.map((order) => <tr key={order.id}><Cell className="font-bold">{order.id}</Cell><Cell>{order.recipientName}</Cell><Cell className="text-muted">{formatDateTime(order.createdAt)}</Cell><Cell className="font-bold">{formatMoney(order.totalSatang)}</Cell><Cell><StatusBadge status={order.status} /></Cell><Cell><Button size="sm" variant="outline">อัปเดตสถานะ</Button></Cell></tr>)}
      </DataTable>
    </OperationsShell>
  );
}
